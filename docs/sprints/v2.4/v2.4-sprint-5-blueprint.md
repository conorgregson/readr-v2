# Sprint 5 Blueprint — Hardening, Accessibility & Release Lock

Readr v2.4

Objective:
Stabilize the full engagement and insights layer for release.

---

# Sprint 5 Goal

- No CRUD regressions
- Accessibility baseline preserved
- Large-library performance validated
- Bulk mutation recovery flows finalized
- Release docs updated
- v2.4 ready for tag

---

# Scope

## Regression Validation

Re-run:

- Books CRUD
- Sessions CRUD
- Auth/session restore
- Bulk edit flows
- Saved view flows
- Dashboard rendering
- Goal/streak/badge surfaces
- Undo behavior

---

## Hardening

Verify:

- Accessibility of charts and progress indicators
- Keyboard flows for batch actions and saved view management
- Empty/error/loading states across new surfaces
- API contract consistency
- No boundary violations between UI, store, service, and API
- Real restore behavior for bulk status undo
- No client/server drift after grouped status undo
- Stable grouped mutation behavior after refresh/reload

---

## Bulk Mutation Recovery

- Implement real undo for bulk status updates
- Restore mixed prior statuses safely
- Preserve filter/search/order stability after grouped restore
- Ensure grouped undo behavior is server-consistent, not local-only

---

## Performance

- Validate behavior with 2,000+ books
- Confirm dashboard queries remain acceptable
- Confirm bulk operations remain stable under larger datasets

---

## Release Prep

- Update README
- Update roadmap
- Update changelog
- Prepare release notes
- Clean temporary scaffolding or feature flags if used

---

# Note

- Revisit saved view overwrite behavior after Current view drift.
- Evaluate whether overwrite should use `lastAppliedViewId` or a dedicated “Overwrite previous view” flow.

--

# Acceptance Criteria

- No regression in CRUD or ownership-sensitive flows
- Accessibility baseline preserved across new features
- Bulk status undo restores correct server-backed state
- No client/server drift after grouped mutations
- Performance acceptable with large libraries
- Documentation updated for v2.4
- v2.4 ready for official release
